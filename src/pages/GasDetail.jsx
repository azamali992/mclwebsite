...
// Trims a gas's real scraped description down to a clean sentence boundary
// for use as a meta description — never invents or rewrites the underlying
// fact, only shortens where the source text runs past typical SERP length.
function toMetaDescription(text, maxLen = 160) {
...
export default function GasDetail() {
  ...
  const gasUrl = `/gases/${gas.categoryPath}/${gas.slug}`;
  // Schema.org Product, built only from fields that already exist on the
  // parsed gas record (name/formula/description/category/purity grades) —
  // no price/availability/SKU is invented since none exists anywhere in the
  // real dataset for gases.
  const productJsonLd = {
    ...
    ...(gas.formula && gas.formula !== gas.name
      ? { additionalProperty: { '@type': 'PropertyValue', name: 'Chemical formula', value: gas.formula } }
      : {}),
    ...(hasGrades
      ? {
          additionalProperty: gas.purity_grades.map((g) => ({
            '@type': 'PropertyValue',
            name: g.sector ? `Purity (${g.sector})` : 'Purity',
            value: g.purity,
          })),
        }
      : {}),
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: gas.category, item: `${SITE_URL}/gases` },
      { '@type': 'ListItem', position: 3, name: gas.name, item: `${SITE_URL}${gasUrl}` },
    ],
  };
  ...
  return (
    <div className="pt-24">
      <Seo
        title={`${gas.name}${!sameFormula ? ` (${gas.formula})` : ''} Supplier in Pakistan`}
        description={toMetaDescription(gas.description) || `${gas.name} supplied by Multan Chemicals Limited across Pakistan — ${gas.category.toLowerCase()}.`}
        path={gasUrl}
        type="product"
      ...
      <div className="bg-gray-50 border-b border-gray-100 px-4 sm:px-8 lg:px-12 py-3">
        <div className="max-w-[1400px] mx-auto flex items-center gap-2 text-xs text-gray-500">
          ...
          <Link to={`/gases#${gas.section}`} className="hover:text-accent">{gas.category}</Link>
          <FaChevronRight size={8} />
      ...
      <section className="relative w-full bg-slate-900 py-20 px-4 sm:px-8 lg:px-12 overflow-hidden">
        <div className="absolute top-10 right-10 w-72 h-72 rounded-full bg-accent/10 blur-3xl pointer-events-none" />
          ...
          <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-8">
            <div className="max-w-3xl">
              <h1 className="text-white font-extrabold text-4xl lg:text-6xl leading-tight">
                {gas.name}{!sameFormula && <span className="text-accent"> ({gas.formula})</span>}
              </h1>
              ...
              <div className="flex flex-wrap gap-4 mt-8">
                <button onClick={() => navigate('/contact')} className="bg-accent hover:bg-red-700 text-white font-bold text-sm uppercase tracking-wider px-7 py-3.5 rounded inline-flex items-center gap-2 transition-all hover:shadow-lg hover:shadow-red-900/30 active:scale-95 focus:outline-none focus:ring-2 focus:ring-red-500">
                  <FaPhone size={13} /> Request Quote
                </button>
                {hasGrades && (
                  <button onClick={scrollToGrades} className="border-2 border-white/20 text-white hover:border-accent hover:text-accent font-bold text-sm uppercase tracking-wider px-7 py-3.5 rounded inline-flex items-center gap-2 transition-all focus:outline-none focus:ring-2 focus:ring-accent">
            ...
            <div className="w-28 h-28 lg:w-36 lg:h-36 rounded-full bg-white/5 border border-white/10 flex items-center justify-center flex-shrink-0">
              <span className="text-accent font-black text-3xl lg:text-4xl">{gas.formula}</span>
            </div>
          </div>
      ...
      {hasGrades && (
        <section id="grades" className="py-16 px-4 sm:px-8 lg:px-12 bg-white scroll-mt-28">
          <div className="max-w-[1400px] mx-auto">
            <p className="text-accent font-bold uppercase tracking-widest text-sm mb-2">Purity Standards</p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
              {gas.purity_grades.map((g, i) => (
                <div key={i} className="border-2 border-gray-100 hover:border-accent rounded-xl p-6 transition-colors">
                  <p className="text-accent font-bold uppercase tracking-widest text-xs mb-2">{g.sector}</p>
      ...
      {(hasTechSpecs || hasCylinders || hasBulk) && (
        <section className="py-16 px-4 sm:px-8 lg:px-12 bg-gray-50">
          <div className="max-w-[1400px] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12">
            {hasTechSpecs && (
              <div>
                <p className="text-accent font-bold uppercase tracking-widest text-sm mb-2">Technical Specifications</p>
            ...
            {(hasCylinders || hasBulk) && (
              <div>
                <p className="text-accent font-bold uppercase tracking-widest text-sm mb-2">Packaging &amp; Delivery</p>
                      ...
                      <tbody className="divide-y divide-gray-100">
                        {gas.cylinders.map((c, i) => (
                          <tr key={i}>
                            ...
                            <td className="px-5 py-3 text-accent font-semibold whitespace-nowrap">{c.pressure}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
      ...
      {hasUseCases && (
        <section className="py-16 px-4 sm:px-8 lg:px-12 bg-white">
          <div className="max-w-[1400px] mx-auto">
            <p className="text-accent font-bold uppercase tracking-widest text-sm mb-2">Use Cases</p>
            <h2 className="text-gray-900 font-extrabold text-3xl mb-10">Where {gas.name} Is Used</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {gas.useCases.map((uc, i) => (
                <div key={i} className="border border-gray-100 rounded-xl p-6 hover:border-accent hover:shadow-md transition-all">
                  <div className="w-9 h-9 rounded-lg bg-red-50 text-accent flex items-center justify-center font-bold text-xs mb-4">{String(i + 1).padStart(2, '0')}</div>
          ...
          <div className="max-w-[1400px] mx-auto flex flex-wrap items-center gap-3 justify-center">
            {[...new Set(gas.certifications)].map((c, i) => (
              <span key={i} className="flex items-center gap-2 bg-white border border-gray-200 rounded-full px-4 py-2 text-xs font-semibold text-gray-700">
                <FaAward className="text-accent" size={12} /> {c}
              </span>
            ))}
          </div>
        </section>
            ...
            <div className="flex flex-wrap gap-4">
              {related.map((r) => (
                <Link
                  ...
                  className="group flex items-center gap-3 border border-gray-200 hover:border-accent rounded-xl px-5 py-3.5 transition-all hover:shadow-md"
                >
                  <span className="w-9 h-9 rounded-full bg-gray-50 group-hover:bg-red-50 text-accent font-bold text-xs flex items-center justify-center flex-shrink-0">{r.formula}</span>
                  <span className="text-gray-800 font-semibold text-sm group-hover:text-accent">{r.name}</span>
                  <FaArrowRight className="text-accent opacity-0 group-hover:opacity-100 transition-opacity" size={11} />
                </Link>
              ))}
            </div>
          </div>
      ...
      <section className="py-16 px-4 sm:px-8 lg:px-12 bg-gradient-to-r from-accent to-red-700">
        <div className="max-w-[1400px] mx-auto text-center">
          ...
          <button onClick={() => navigate('/contact')} className="bg-white text-accent hover:bg-gray-100 font-bold text-sm uppercase tracking-wider px-8 py-3.5 rounded inline-flex items-center gap-2 transition-all active:scale-95 focus:outline-none focus:ring-2 focus:ring-white">
