...
function JobCard({ job, onApply }) {
  return (
    <div className="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden hover:shadow-xl transition-all group">
      <div className="bg-gradient-to-r from-accent to-red-700 p-6 text-white">
      ...
      <div className="p-6">
            ...
            <ul className="space-y-2">
              {(job.responsibilities || []).slice(0, 2).map((item, i) => (
                <li key={i} className="text-gray-600 text-xs flex items-start gap-2">
                  <span className="text-accent mt-1">•</span><span>{item}</span>
                </li>
              ))}
              {(job.responsibilities || []).length > 2 && (
                <li className="text-accent text-xs font-semibold">+{(job.responsibilities || []).length - 2} more</li>
              )}
            </ul>
          </div>
          <div>
            ...
            <ul className="space-y-2">
              ...
              {(job.requirements || []).length > 2 && (
                <li className="text-accent text-xs font-semibold">+{(job.requirements || []).length - 2} more</li>
              )}
            </ul>
          </div>
        </div>
        <div className="border-t border-gray-200 pt-4 flex items-center justify-between">
          ...
          <button
            onClick={() => onApply(job)}
            className="bg-accent hover:bg-red-800 text-white px-6 py-2 text-xs font-bold uppercase tracking-wider transition-all inline-flex items-center gap-2 focus:ring-2 focus:ring-red-500 focus:outline-none rounded"
          >
...
export default function Careers() {
  ...
  return (
    <div className="pt-24">
      <Seo
        title="Careers"
        description="Join Multan Chemicals Limited's growing team: explore open roles in gas operations, medical gas engineering, sales and quality assurance across Pakistan, and learn about our culture, training and benefits."
        path="/careers"
      />
      <section className="bg-gradient-to-r from-slate-900 to-slate-800 py-20 px-4 sm:px-8 lg:px-12">
        <div className="max-w-[1400px] mx-auto text-center">
          <p className="text-accent font-bold uppercase tracking-widest text-sm mb-3">Build Your Career</p>
      ...
      <section className="py-20 bg-gray-50 px-4 sm:px-8 lg:px-12">
        <div className="max-w-[1400px] mx-auto">
          <div className="text-center mb-16">
            <p className="text-accent font-bold uppercase tracking-widest text-sm mb-3">Our Culture</p>
              ...
              <ul className="space-y-3">
                {['Continuous learning and skill development', 'Leadership training programs', 'Industry certifications and courses', 'Career advancement opportunities'].map((item, i) => (
                  <li key={i} className="flex items-center gap-3 text-gray-700">
                    <span className="w-2 h-2 rounded-full bg-accent flex-shrink-0" />{item}
                  </li>
                ))}
              </ul>
            </div>
      ...
      <section className="py-20 px-4 sm:px-8 lg:px-12 bg-white">
        <div className="max-w-[1400px] mx-auto">
          <div className="text-center mb-16">
            <p className="text-accent font-bold uppercase tracking-widest text-sm mb-3">Opportunities</p>
      ...
      <section className="py-16 bg-gradient-to-r from-accent to-red-700 px-4 sm:px-8 lg:px-12">
        <div className="max-w-[1400px] mx-auto text-center">
          ...
          <button onClick={() => navigate('/contact')} className="bg-white hover:bg-gray-100 text-accent px-8 py-3.5 text-sm font-bold uppercase tracking-wider transition-all inline-flex items-center gap-2 focus:ring-2 focus:ring-white focus:outline-none rounded">
      ...
      {selectedJob && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="bg-gradient-to-r from-accent to-red-700 p-6 text-white flex justify-between items-start">
              <div>
                    ...
                    <div>
                      ...
                      <ul className="space-y-2">
                        {(selectedJob.responsibilities || []).map((item, i) => (
                          <li key={i} className="text-gray-600 text-sm flex items-start gap-2"><span className="text-accent mt-1 flex-shrink-0">✓</span><span>{item}</span></li>
                        ))}
                      </ul>
                    </div>
                    <div>
                  ...
                  <div className="border-t border-gray-200 pt-6">
                    ...
                    <form onSubmit={handleApplySubmit} className="space-y-4">
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                          <label htmlFor="fullname" className="block text-xs font-semibold text-gray-700 mb-2"><span className="text-accent">*</span> Full Name</label>
                          <input id="fullname" type="text" required value={form.fullname} onChange={e => setForm({...form, fullname: e.target.value})} placeholder="Your name" className="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm outline-none focus:border-accent focus:ring-1 focus:ring-accent" />
                        </div>
                        <div>
                          <label htmlFor="email" className="block text-xs font-semibold text-gray-700 mb-2"><span className="text-accent">*</span> Email</label>
                          <input id="email" type="email" required value={form.email} onChange={e => setForm({...form, email: e.target.value})} placeholder="your@email.com" className="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm outline-none focus:border-accent focus:ring-1 focus:ring-accent" />
                        </div>
                      </div>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                          <label htmlFor="phone" className="block text-xs font-semibold text-gray-700 mb-2">Phone</label>
                          <input id="phone" type="tel" value={form.phone} onChange={e => setForm({...form, phone: e.target.value})} placeholder="+92 300 1234567" className="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm outline-none focus:border-accent focus:ring-1 focus:ring-accent" />
                        </div>
                        <div>
                          <label htmlFor="experience" className="block text-xs font-semibold text-gray-700 mb-2">Years of Experience</label>
                          <input id="experience" type="number" value={form.experience} onChange={e => setForm({...form, experience: e.target.value})} placeholder="e.g., 5" className="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm outline-none focus:border-accent focus:ring-1 focus:ring-accent" />
                        </div>
                      </div>
                      <div>
                        <label htmlFor="message" className="block text-xs font-semibold text-gray-700 mb-2"><span className="text-accent">*</span> Cover Letter</label>
                        <textarea id="message" required rows={4} value={form.message} onChange={e => setForm({...form, message: e.target.value})} placeholder="Tell us why you're a great fit..." className="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm outline-none focus:border-accent focus:ring-1 focus:ring-accent resize-none" />
                      </div>
                      <div>
                        ...
                        <input
                          ...
                          className="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm outline-none focus:border-accent focus:ring-1 focus:ring-accent file:mr-3 file:py-1 file:px-3 file:rounded file:border-0 file:bg-gray-100 file:text-gray-700 file:text-xs"
                        />
                      </div>
                      <div className="flex gap-4 justify-end pt-4 border-t border-gray-200">
                        <button type="button" onClick={closeModal} className="px-6 py-2.5 border border-gray-300 text-gray-700 font-semibold rounded-lg hover:bg-gray-50 transition-colors focus:outline-none focus:ring-2 focus:ring-accent">Cancel</button>
                        <button type="submit" disabled={loading} className="px-6 py-2.5 bg-accent hover:bg-red-800 disabled:bg-gray-400 text-white font-semibold rounded-lg transition-colors focus:outline-none focus:ring-2 focus:ring-red-500">{loading ? 'Submitting...' : 'Submit Application'}</button>
                      </div>
                    </form>
                  </div>
                </>
